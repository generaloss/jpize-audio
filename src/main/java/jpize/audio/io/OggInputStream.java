...
public class OggInputStream extends AudioInputStream {
    ...
    private final static boolean IS_BIG_ENDIAN = (ByteOrder.nativeOrder() == ByteOrder.BIG_ENDIAN);
    ...
    public OggInputStream(InputStream input) {
        ...
        readPCM();
        super.setup(oggInfo.channels, 16, oggInfo.rate);
    }
        ...
        while(true){ // we repeat if the bitstream is chained
            ...
            while(!endOfBitStream){
                while(!endOfBitStream){
                    ...
                    if(result == -1){ // missing or corrupt data at this page position
                        ...
                        while(true){
                            ...
                            if(result == 0)
                                break; // need more data
                            if(result != -1){ // -1 missing or corrupt data at this page position
                ...
                if(!endOfBitStream){
                    ...
                    if(index >= 0){
                        try{
                            bytes = in.read(syncState.data, index, BUFFER_SIZE);
                        }catch(IOException e){
    ...
    private boolean getPageAndPacket() {
        ...
        try{
            bytes = in.read(buffer, index, BUFFER_SIZE);
        }catch(Exception e){
