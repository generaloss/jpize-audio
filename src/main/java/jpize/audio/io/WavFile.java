    ...
    public void save() {
        try(final DataOutputStream out = new DataOutputStream(new FileOutputStream(file))){
            ...
            { // Chunk Descriptor
                ...
                out.writeInt(Integer.reverseBytes(20 + 16 + Integer.reverseBytes(data.length))); // Size (Not necessarily)
            ...
            { // Ftm SubChunk
                ...
                out.writeInt(Integer.reverseBytes(sampleRate * channels * BITS_PER_SAMPLE / 8)); // Byte Rate (Not necessarily)
